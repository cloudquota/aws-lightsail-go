...
func FetchMonthlyCreditUsage(ctx context.Context, ak, sk, proxy string) (CreditSummary, error) {
	...
	endpoint := strings.TrimSpace(os.Getenv("COST_EXPLORER_ENDPOINT"))
	if endpoint == "" {
		endpoint = "https://ce.us-east-1.amazonaws.com"
	}
	endpointRegion := strings.TrimSpace(os.Getenv("COST_EXPLORER_REGION"))
	if endpointRegion == "" {
	...
	signer := v4.NewSigner()
	if err := signer.SignHTTP(ctx, creds, req, hex.EncodeToString(hash[:]), "ce", endpointRegion, time.Now().UTC()); err != nil {
		return CreditSummary{}, err
	}
